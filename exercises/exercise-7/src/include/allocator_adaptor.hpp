...
namespace numa
{
    ...
    public:
        ...
        void construct(U *ptr, ArgsT &&...args)
        {
            if constexpr (std::is_trivially_constructible_v<U> && sizeof...(args) == 0)
            {
                // skip construction: no-init for trivially constructible, no-args
            }
            else
            {
    ...
    struct alignas(ALIGNMENT) aligned_class : public T
    {
        static_assert(ALIGNMENT >= alignof(T));
        template <class... Args>
        aligned_class(Args &&...args) : T{std::forward<Args>(args)...} {}
    };
    ...
    struct alignas(ALIGNMENT) aligned_nonclass
    {
        static_assert(ALIGNMENT >= alignof(T));
        template <class... Args>
        aligned_nonclass(Args &&...args) : a{std::forward<Args>(args)...} {}
